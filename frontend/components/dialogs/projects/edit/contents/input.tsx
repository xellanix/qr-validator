...
function AllowDuplicateValidAction() {
    const allowDuplicateValid = useProjectStore((s) => s.edit.data?.allowDuplicateValid || false);
    ...
    const checkChanged = (v: boolean) => {
        useProjectStore.setState((s) => {
            ...
            return {
                edit: {
                    ...s.edit,
                    data: { ...s.edit.data, allowDuplicateValid: v, isContinuousScanning },
                },
            };
        });
    };
...
function MaxValidDuplicateAction() {
    const maxValidDuplicate = useProjectStore((s) =>
        Math.max(2, s.edit.data?.maxValidDuplicate || 2),
    );
    ...
    const inputChanged = (v: string) => {
        useProjectStore.setState((s) => {
            ...
            return {
                edit: { ...s.edit, data: { ...s.edit.data, maxValidDuplicate: parseInt(v) } },
            };
        });
    };
...
function ContinuousScanningAction() {
    const allowDuplicateValid = useProjectStore((s) => s.edit.data?.allowDuplicateValid || false);
    const continuousScanning = useProjectStore((s) => s.edit.data?.isContinuousScanning ?? true);

    const checkChanged = (v: boolean) => {
        useProjectStore.setState((s) => {
            if (!s.edit.data) return s;

            const isContinuousScanning = s.edit.data.allowDuplicateValid ? false : v;
            return { edit: { ...s.edit, data: { ...s.edit.data, isContinuousScanning } } };
        });
    };
