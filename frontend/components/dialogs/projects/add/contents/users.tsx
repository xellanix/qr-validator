...
export function AssignUsersSection() {
    return (
...
function AssignedUsers() {
    const users = useProjectStore((s) => s.newProject?.data?.users);

    return (
        <ItemGroup className="*:rounded-t-none *:border-t-0 *:not-last:rounded-b-none gap-0!">
            <Administrator />
...
const setUsers = (prev: (prev: User[]) => User[]) => {
    useProjectStore.setState((s) => {
        ...
        return { newProject: { ...newProject, data: { ...data, users } } };
    });
};
...
export function ReadOnlyAssignedUsers() {
    const users = useProjectStore((s) => s.newProject?.data?.users);
    ...
    return (
        <ItemGroup className="*:rounded-t-none *:border-t-0 *:not-last:rounded-b-none gap-0!">
            ...
            {users?.map(({ name, authorizeLevel }, i) => (
                <Item key={`(${name}, ${authorizeLevel})[${i}]`} variant={"outline"}>
                    <ItemContent>
                        <ItemTitle>{name}</ItemTitle>
                    </ItemContent>
                    <ItemActions className="font-mono">{getUserRole(authorizeLevel)}</ItemActions>
                </Item>
            ))}
        </ItemGroup>
    );
