            ...
            <ItemGroup className="*:not-first:rounded-t-none *:not-first:border-t-0 *:not-last:rounded-b-none gap-0">
                <Alert>
                    ...
                    <AlertDescription className="text-wrap">
                        You can set up specific rules here to control what kind of data is allowed
                        into your system. If you don't need strict rules, feel free to skip this
                        step—by default, the system will accept and approve any input you give it.
                    </AlertDescription>
                </Alert>
            </ItemGroup>
            <ItemGroup className="*:not-first:rounded-t-none *:not-first:border-t-0 *:not-last:rounded-b-none gap-0!">
...
function InputTestAction() {
    ...
    const tryValidate = () => {
        ...
        const schema = useProjectStore.getState().newProject?.data?.schema;
        if (!value || !schema) {
...
function SchemaAction() {
    const addSchema = useCallback(
        (type: string) => () => {
            ...
            useProjectStore.getState().updateNewProjectSchema((prev) => [...prev, newValue]);
        },
        [],
    );
...
export function ActiveSchemasFlow() {
    const schemaObjects = useProjectStore((s) => s.newProject?.data?.schemaObjects);
...
function SchemaList() {
    ...
    const onMoved = useCallback((ev: { activeIndex: number; overIndex: number }) => {
        const { updateNewProjectSchema } = useProjectStore.getState();
        updateNewProjectSchema((prev) => arrayMove(prev, ev.activeIndex, ev.overIndex));
    }, []);
...
function SchemaItem({ schema }: { schema: SchemaObjectSortable }) {
    ...
    const removeSchema = () => {
        const { updateNewProjectSchema } = useProjectStore.getState();
        updateNewProjectSchema((prev) => prev.filter((s) => s.sortId !== schema.sortId));
    };
